<template>
  <div class="staff-list-container app-page">
    <div class="page-header app-page-header">
      <div class="header-content app-page-header-content">
        <div class="title-section app-page-heading">
          <h1>
            <el-icon class="title-icon app-title-icon"><User /></el-icon>
            物业员工管理
          </h1>
          <p>统一管理员工账号与岗位信息</p>
        </div>
    ...
    <el-card class="table-card app-table-card" shadow="never">
      ...
      <div v-if="!staffStore.loading && !staffStore.hasStaffs" class="empty-state">
        <el-empty description="暂无员工数据">
          <el-button type="primary" @click="handleAdd">新增第一个员工</el-button>
        </el-empty>
      </div>
    </el-card>
    ...
    >
      <el-form ref="formRef" :model="formData" :rules="formRules" label-width="100px">
        ...
        <el-form-item label="岗位" prop="position">
          <el-input v-model="formData.position" maxlength="50" placeholder="如：客服 / 经理" />
        </el-form-item>

        <el-form-item label="小区名" prop="property_name">
          <el-input v-model="formData.property_name" maxlength="100" placeholder="请输入物业名称" />
        </el-form-item>
      ...
      <template #footer>
        <el-button class="app-button app-button-secondary" @click="dialogVisible = false">取消</el-button>
        <el-button class="app-button app-button-primary" :loading="submitting" @click="handleSubmit">
          {{ isEdit ? '更新' : '创建' }}
        </el-button>
      </template>
    </el-dialog>
...
const handleSizeChange = async (pageSize) => {
  await staffStore.changePage(1, pageSize)
}

const handleCurrentChange = async (page) => {
  await staffStore.changePage(page)
}
